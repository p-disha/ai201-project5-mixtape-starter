...
def search_songs(query: str) -> list[dict]:
    ...
    """
    # Match on title or artist only. We deliberately do NOT join song_tags:
    # a song with N tags would produce N joined rows (one per tag), duplicating
    # the song in the results. Tags are loaded per-song by Song.to_dict().
    results = (
